...
def bt_selection_buttons(id_, isCanCncl=True):
    ...
    if config_dict['WEB_PINCODE']:
        buttons.ubutton("Select Files", f"{BASE_URL}/app/files?gid={id_}")
        buttons.ibutton("Pincode", f"btsel pin {gid} {pincode}")
    else:
        buttons.ubutton("Select Files", f"{BASE_URL}/app/files?gid={id_}&pin={pincode}")
    if isCanCncl:
...
def get_readable_message():
    ...
    for download in list(download_dict.values())[STATUS_START:STATUS_LIMIT+STATUS_START]:
        ...
        if config_dict['DELETE_LINKS'] and int(config_dict['AUTO_DELETE_MESSAGE_DURATION']) > 0:
            ...
            msg += f"\n<b>File Name</b> » <code><i>{escape(f'{download.name()}')}</i></code>\n\n"
...
async def set_commands(client):
    if config_dict['SET_COMMANDS']:
        await client.set_bot_commands([
            BotCommand(f'{BotCommands.MirrorCommand[0]}', f'-z-e zip-un,-m multi,-n newname '),
